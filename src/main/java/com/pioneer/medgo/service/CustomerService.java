...
public class CustomerService {
	...
	public List<PharmacyDTO> nearbyPharmacyList(double latitude, double longitude, String keyword) {
		...
		List<PharmacyDTO> list = pharmacyDAO.findByLessThanLatAndLon(minLat, maxLat, minLon, maxLon, keyword);
		...
		list = list.stream().filter(p -> p.getDistance() <= nearDistance * 1000.0)
				.sorted(Comparator.comparingDouble(PharmacyDTO::getDistance)).collect(Collectors.toList());
